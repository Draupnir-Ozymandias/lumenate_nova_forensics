...
| The verified Vitality audio and light program share an app/native media-position association. | SHA-256-identified audio, `syncMe(positionMs)` control flow, runtime BLE/media pairs, and a simultaneous audio-correlated run. | New audio/video anchors carry 20 ms uncertainty and still exclude calibrated firmware-to-photon latency. |
| Vitality physically emits closely matching four-emitter flashes and approximately the reconstructed frequency. | Two full 120 fps recordings, a pilot, and one simultaneous S21+A35 run; the 10.5 Hz segment repeatedly measured about 10.464 Hz. | Direct optical corroboration on a shared clock; camera sampling still prevents a calibrated packet-to-photon claim. |
...
| Firmware 1.0.4 stops a held live-session command shortly after forced BLE loss. | Simultaneous HCI, MediaSession, verified-audio, and S21 optical capture; no zero/inactive write preceded loss. | Three further 10.5 Hz pulses were observed and the final illuminated frame mapped about 229 ms after disconnect; the camera is not a calibrated photon timer. |
...
|---|---|---|---|---|---|
| Vitality, 5 min | Complete 34-segment 7.2.1 declaration | Complete natural run plus 98 s simultaneous run | Verified asset; BLE/media and audio/video anchors | Two full S21 runs, pilot, and simultaneous partial run | `vitality-5min-empirical-0.2.0.json` |
...
| `ave_57528d1a5af4d899` | audio/BLE/optical clock bridge | Vitality simultaneous 2026-09-24 run | 0.99 |
| `ave_3fe239e4a6f8785e` | forced-disconnect watchdog behavior | Vitality / firmware 1.0.4 | 0.98 |
...
| N2 BLE/state machine | Characteristic audit and forced-disconnect watchdog path complete | Characterize isolated packet loss and active-session reconnect; `0x001f` remains firmware-exposed but current-app-unused. |
...
| N4 native/firmware boundary | Bounded report plus firmware 1.0.4 link-loss hold measurement | Device timer implementation and firmware validation/boot policy remain unresolved. |
...
- The 2026-09-24 Vitality capture supplies one bounded BLE/audio/video clock
  chain, but it is not a calibrated photon-level clock.
...
1. version/content drift detection described in
   `NEXT_STEPS_WITHOUT_PHOTODIODE.md`;
